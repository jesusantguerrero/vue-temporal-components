...
// node_modules/vueuse-temporals/dist/index.mjs
...
var Locale = class _Locale {
  ...
  static fromObject({ locale: locale2, numberingSystem, outputCalendar } = {}) {
    return _Locale.create(locale2, numberingSystem, outputCalendar);
  }
...
function removePrecisionIssue(a3) {
  return Math.trunc(a3 * 1e3) / 1e3;
}
function convert(matrix, fromMap, fromUnit, toMap, toUnit) {
  const conv = matrix[toUnit][fromUnit], raw = fromMap[fromUnit] / conv, added = Math.floor(raw);
...
var Duration = class _Duration {
  ...
  toMillis() {
    ...
    for (let unit of reverseUnits.slice(1)) {
      if ((_a = this.values) == null ? void 0 : _a[unit]) {
...
var Interval = class _Interval {
  ...
  intersection(other) {
    ...
    if (s4 >= e2) {
      ...
      return _Interval.fromDateTimes(s4, e2);
    }
  }
  /**
...
function unitForToken(token, loc) {
  const one = digitRegex(loc), two = digitRegex(loc, "{2}"), three = digitRegex(loc, "{3}"), four = digitRegex(loc, "{4}"), six = digitRegex(loc, "{6}"), oneOrTwo = digitRegex(loc, "{1,2}"), oneToThree = digitRegex(loc, "{1,3}"), oneToSix = digitRegex(loc, "{1,6}"), oneToNine = digitRegex(loc, "{1,9}"), twoToFour = digitRegex(loc, "{2,4}"), fourToSix = digitRegex(loc, "{4,6}"), literal = (t2) => ({ regex: RegExp(escapeToken(t2.val)), deser: ([s4]) => s4, literal: true }), unitate = (t2) => {
    if (token.literal) {
      return literal(t2);
    }
    switch (t2.val) {
...
function getCalendarISO(date, nextMode, options = { fixedMonth: false }) {
...
function useDatePager(props) {
  ...
  const getCalendar = (date, nextMode2, options2) => {
    const controls = {
      day: getWeekDays,
    ...
    let mode = nextMode2 || "week";
    if (nextMode2.startsWith("P")) {
      mode = "iso";
    }
    return controls[mode](new Date(date), nextMode2, options2);
  };
  const setMode = (value) => {
  ...
  setDay(props.initialDate || /* @__PURE__ */ new Date());
  watch(
    selectedDay,
    (newValue, oldValue) => {
      if (nextMode.value === "day" && !isSameDate(newValue, oldValue) || nextMode.value === "month" && !isSameMonth(newValue, oldValue)) {
        checkDateSpan();
      }
    },
    { immediate: true }
...
//# sourceMappingURL=vueuse-temporals.js.map
